...
def connect(root: Path) -> duckdb.DuckDBPyConnection:
    ...
    becomes a same-named view over every `*.parquet` file beneath it
    (`data.parquet` for a normal partition, or `part-*.parquet` for one
    built incrementally in chunks - see `write_hive_partitioned`'s
    `part_name`), with the `exchange`/`symbol`/`year`/`month` hive keys
    ...
    if root.exists():
        for dataset_dir in sorted(p for p in root.iterdir() if p.is_dir()):
            ...
            glob_pattern = str(dataset_dir / "**" / "*.parquet").replace("'", "''")
            con.execute(
